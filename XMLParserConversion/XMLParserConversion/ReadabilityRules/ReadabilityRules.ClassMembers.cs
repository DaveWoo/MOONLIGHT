...
namespace StyleCop.CSharp
{
    ...
    {
        public ReadabilityRules(StyleCopCore core)
        {
            this.Core = core;
        }
        ...
        /// </returns>
        public bool DelayAnalysis(CodeDocument document, int passNumber)
        {
        ...
        /// </returns>
        public bool CheckClassMemberRulesForElements(CsElement element, ClassBase parentClass, Dictionary<string, List<CsElement>> members)
        {
            ...
            Dictionary<string, List<CsElement>> members)
        {
            ...
            if (!(tokenNode.Value is TypeToken) || tokenNode.Value.CsTokenClass == CsTokenClass.GenericType)
            {
                ...
                if (!tokenNode.Value.Text.StartsWith(".", StringComparison.Ordinal))
                {
                    ...
                    if (IsLocalMember(memberName, tokenNode.Value, expression))
                    {
                        //CodePartType codePartType = expression.CodePartType;
                        //Expression tempExpression = expression.Parent as Expression;
                        //Expression orignalExpression = expression;
                        ...
                        //}
                        IParserObject itemElement = Common.ConvertStyleCopTypeToLocal(expression, parentElement);

                        if (itemElement != null)
                            Common.ContextXMLParserList.Add(itemElement);
                    }
                }
            }
        }
